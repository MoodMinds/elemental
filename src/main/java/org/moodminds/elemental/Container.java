...
/**
 * A Container represents a holder of objects referred to as its elements.
 * <p>
 * A Container serves as a holder for zero, one, or more elements. Whether it is
...
public interface Container<V> extends Iterable<V> {
     ...
     * <p>
     * {@implNote} default is implemented as filtering over Container iterator
     */
    default Iterator<V> getAll(Object o) {

        Iterator<V> iterator = iterator(); Predicate<V> predicate = isEqual(o);
    ...
    default int getCount(Object o) {
        int[] count = new int[]{0};
        getAll(o).forEachRemaining(v -> count[0]++);
        return count[0];
    }

    /**
     * Return the number of elements in this Container. If this Container contains
     * more than {@link Integer#MAX_VALUE} elements, returns {@link Integer#MAX_VALUE}.
     *
