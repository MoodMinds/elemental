 ...
 * @see RandomGet
 */
public interface Sequence<V> extends Ended<V>, Ordered<V> {
     ...
     */
    Sequence<V> sub(int fromIndex, int toIndex);
     ...
     */
    @Override
    default SequenceIterator<V> iterator() {
        return iterator(0);
    }
     ...
     */
    SequenceIterator<V> iterator(int index);

    /**
     * Returns a {@link SequenceIterator} that provides elements in reverse order, from the last (tail).
     *
     * @return a {@link SequenceIterator} for the elements in reverse order (from the tail)
     */
    @Override
    default SequenceIterator<V> descendingIterator() {
        return new SequenceIterator<V>() {

            final SequenceIterator<V> iterator = iterator(size());
