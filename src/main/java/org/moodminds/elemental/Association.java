...
public interface Association<K, V, KV extends KeyValue<K, V>> extends Container<KV> {
    ...
    default boolean contains(Object key, Object value) {
        return Objects.equals(get(key), value) && (value != null || containsKey(key));
    }
    ...
    /**
     * Perform the specified action for each {@link KV} entry in this map until either
     ...
     *
     * @implSpec
     * The default implementation is equivalent to the following for this {@code map}:
     * <pre>{@code
     *      map.forEach(kv -> action.accept(kv.getKey(), kv.getValue()));
     * }</pre>
     *
